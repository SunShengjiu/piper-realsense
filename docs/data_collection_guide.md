...
不是一帧。目标是采集约 150 个 episode，每个 episode 不限时间。启用
`--return-on-q` 后，程序监听交互终端的 `q`；按下后保存当前 episode，再按“重置到待机 → CAN”
流程把六个关节移动到配置的采集初始位姿 `[90,0,0,0,0,0]°`。按钮结束记录本身不会把
`ctrl_mode` 直接改成 CAN。`--arm-button-return-zero` 仍可兼容机械臂示教按钮。
...
2. **按空格**启动当前 episode。相机、机械臂和写入器需要初始化；等子进程显示
   ...
   只表示启动请求已发出。
3. 完成本次演示后，按 `q` 结束当前 episode；先等待数据保存，再重置到待机、切换 CAN 并等待
   机械臂回到采集初始位姿。
4. **等待“当前 episode 已保存，机械臂已回到配置初始位姿”及下一次“等待开始”**。保存和回位
   期间不要再次按键或更换场景。
5. 看到等待提示后再更换场景，按空格开始下一个 episode，重复上述步骤。

**q 工作流顺序：摆场景 → 空格 → 等待开始采集 → 演示 → 按 q → 等待保存、退出示教和回位 → 换场景 → 空格。**
不要在回零完成前换场景，否则下一条场景可能与上一条收尾过程混在一起。

| 当前阶段 | 空格 | `q` | Ctrl-C |
| --- | --- | --- | --- |
| 等待开始 | 启动下一条 | 退出整个交互程序 | 退出整个交互程序 |
| 正在采集 | 无操作 | 结束并保存当前条，然后自动回到初始位姿 | 请求结束当前条并退出程序 |
| 正在保存 | 等待完成后再操作 | 等待完成后再操作 | 建议等待保存完再退出 |

启用自动回位时，`q` 是每条 episode 的结束键。程序先结束采集和视频编码，再发送 ResetPiper
退出示教、选择 CAN，最后发位置控制目标；
回零默认速度 20%，误差阈值 ±1°，超时 20 秒。ResetPiper 可能短暂释放电机使能，执行前确认
机械臂有支撑且急停可用。
“回零”表示移动到目标姿态，不会调用 `JointConfig(set_zero=0xAE)`，所以不会改写电机的
持久化零点。检测到机械臂不在 CAN 模式、状态异常、反馈超时或无法到位时，程序停止发送
目标并返回 `failed_return_to_zero`，请人工确认机械臂状态后再重试。
